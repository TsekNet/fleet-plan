...
| `GET` | `/api/v1/fleet/policies` | Global policies (when default.yml parsed) |
...
Global endpoints (`/config`, `/policies`, `/queries` with teamID=0) are only called when `default.yml` defines global sections.
